...
import { getInspection, setLastSaveText, getLastLocalSaveAt, setLastLocalSaveAt } from './state.js?v=256';
...
function showSaveError(msg) {
  if (typeof _onSyncStatusChange === 'function') _onSyncStatusChange('failed');
...
export async function saveNow(options) {
  ...
  try {
    ...
    if (typeof _onSyncStatusChange === 'function') _onSyncStatusChange('local'); // Change 2
...
export function backupToLocalStorage() {
  ...
  try {
    ...
    if (bak.rapidCaptureDraft && Array.isArray(bak.rapidCaptureDraft.photos)) {
      bak.rapidCaptureDraft.photos = bak.rapidCaptureDraft.photos.map(p => ({
        photoId: p.photoId,
        stepName: p.stepName,
        roomName: p.roomName,
        caption: p.caption,
