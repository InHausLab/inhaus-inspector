    ...
    switch (f.type) {
      ...
      case 'ai-room-summary': {
        ...
        function buildSummaryUI() {
          ...
          genBtn.textContent = '\uD83D\uDCDD Polish Inspector Notes';
          genBtn.disabled = !String(data.notes || '').trim();

          const ta = document.createElement('textarea');
          ta.className = 'field-textarea ai-room-summary-textarea';
          ta.placeholder = 'Write room notes first, then use AI to polish the same meaning...';
          ...
          hint.textContent = String(data.notes || '').trim()
            ? 'AI polishes the inspector notes already written for this room.'
            : 'Write or dictate room notes first. AI will not create the first draft.';

          async function doGenerate() {
            const inspectorNotes = String(data.notes || '').trim();
            if (!inspectorNotes) {
              genBtn.disabled = true;
              showToast('Write room notes first, then use AI to polish them');
              return;
            }
            genBtn.disabled = true;
            genBtn.textContent = '\u23F3 Polishing\u2026';
            ...
            const prompt = 'You are editing inspector-written room notes for a home health inspection report.' +
              ' Room: ' + (summaryData.roomName || 'Unknown') +
              '. Supporting app fields for context only: ' + JSON.stringify(meaningful) +
              '\n\nInspector notes to polish:\n' + inspectorNotes +
              '\n\nRewrite the inspector notes in 1-3 clear, factual sentences. Preserve the same meaning and priorities. Do not add new findings, concerns, room names, recommendations, tests, values, or timeframes that are not already in the inspector notes. Use calm language. No bullet points, no headers, no markdown. If the notes say no issue was found, keep that meaning.';

            try {
              const resp = await fetchWithTimeout(PROXY_URL, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: JSON.stringify({ prompt, _syncSecret: 'ihl-sync-2026' })
              ...
              ta.value = text;
              changed();
              genBtn.textContent = '\u2713 Notes Polished';
              setTimeout(() => {
                genBtn.disabled = false;
                genBtn.textContent = '\u21BA Polish Again';
              }, 2000);
            } catch (err) {
              genBtn.disabled = false;
              genBtn.textContent = '\uD83D\uDCDD Polish Inspector Notes';
          ...
          if (data.aiSummary) {
            ...
            regenLink.textContent = 'Polish again';
